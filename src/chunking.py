...
class RecursiveChunker:
    ...
    def chunk(self, text: str) -> list[str]:
        return self._split(text.strip(), self.separators) if text.strip() else []
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        for part in parts:
            if not part:
                continue
            if len((pending + part).strip()) <= self.chunk_size:
                pending += part
                continue
            if pending.strip():
...
class ChunkingStrategyComparator:
    ...
    def compare(self, text: str, chunk_size: int = 200) -> dict:
        ...
        strategies = {
            "fixed_size": FixedSizeChunker(size, min(50, size - 1)),
            "by_sentences": SentenceChunker(),
            "recursive": RecursiveChunker(chunk_size=size),
        }
        comparison = {}
